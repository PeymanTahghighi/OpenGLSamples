...
void main()
{
	...
	vec3 pointlightVec = normalize(worldPosition - pointLight.lightPosition);
	vec3 spotlightVec = normalize(worldPosition - spotLight.lightPosition);
	vec3 cameraToFragment = normalize(worldPosition - cameraPosition);
	vec3 halfwayPointLight = normalize(-pointlightVec + -cameraToFragment);
	vec3 halfwaySpotLight = normalize(-spotlightVec + -cameraToFragment);
	//----------------------------------------------------------------------

	//apply texture
	vec3 finalColor = texture(texture0,texCoord).rgb;
	if(equalZero(finalColor))
	{
	...
	float attenuation = doPointLight(pointlightVec);
	float pointLightShadow = calculateShadow(depthmapPointlight, worldPosition,pointLight.lightPosition,pointlightVec);
	vec3 specularPointLight = pow(max(dot(halfwayPointLight,normalWorld),0.0f),material.shininess) * pointLight.lightColor * material.specular * attenuation;
	vec3 diffusePointLight = max(dot(-pointlightVec,normalWorld),0.0f) *  pointLight.lightColor * material.diffuse * attenuation;
	specularPointLight *= pointLightShadow;
	diffusePointLight *= pointLightShadow;
	//------------------------------------------------------------------------

	//calculate spotlight
	float spotLightShadow = calculateShadow(depthmapSpotlight, worldPosition,spotLight.lightPosition,spotlightVec);
	float intensity = doSpotLight(spotLight.lightDirection,spotlightVec,spotLight.lightFallOff,spotLight.lightFallOffOffset);
	vec3 specularSpotLight = pow(max(dot(normalWorld,halfwaySpotLight),0.0f),material.shininess) * spotLight.lightColor * material.specular;
	vec3 diffuseSpotLight = max(dot(normalWorld,-spotlightVec),0.0f) * spotLight.lightColor * material.diffuse;
	specularSpotLight = specularSpotLight  * spotLightShadow * intensity;
	diffuseSpotLight = diffuseSpotLight  * spotLightShadow * intensity;
	//------------------------------------------------------------------------

	float ambientStrength = 0.05f;
	vec3 ambient = material.diffuse * pointLight.lightColor * ambientStrength;

	vec3 diffuse =  diffuseSpotLight + diffusePointLight;
	vec3 specular =  specularSpotLight + specularPointLight;
